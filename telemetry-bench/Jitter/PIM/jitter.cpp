...
void calculateRTTandJitter(uint64_t maxPackets, vector<int> &synSecs, vector<int> &synUsecs,
                           vector<int> &ackSecs, vector<int> &ackUsecs, int64_t &sumRTT, int64_t &sumJitter)
{
    // Allocate PIM objects for timestamps and RTT
    ...
    PimObjId shiftedRTT_pim = pimAllocAssociated(synSec_pim, PIM_INT32);
    PimObjId jitter_pim = pimAllocAssociated(synSec_pim, PIM_INT32);

    if (synSec_pim == -1 || synUsec_pim == -1 || ackSec_pim == -1 || ackUsec_pim == -1 ||
        rtt_pim == -1 || shiftedRTT_pim == -1 || jitter_pim == -1)
    {
    ...
    // Calculate Jitter
    pimCopyDeviceToDevice(rtt_pim, shiftedRTT_pim);    // Copy RTT values to shiftedRTT_pim
    pimShiftElementsRight(shiftedRTT_pim);            // Shift RTT values to the right
    pimSub(rtt_pim, shiftedRTT_pim, jitter_pim);      // Jitter = RTT - ShiftedRTT
    pimAbs(jitter_pim, jitter_pim);                   // Absolute value of jitter

    // Perform reduction to calculate the sum of RTTs and Jitters
    PimStatus status = pimRedSumInt(rtt_pim, &sumRTT);
    if (status != PIM_OK)
    {
        cerr << "PIM reduction failed for RTT!" << endl;
    }

    status = pimRedSumInt(jitter_pim, &sumJitter);
    if (status != PIM_OK)
    {
        cerr << "PIM reduction failed for Jitter!" << endl;
    }
...
int main()
{
    ...
    int64_t totalRTT = 0, totalJitter = 0;
    calculateRTTandJitter(synSecs.size(), synSecs, synUsecs, ackSecs, ackUsecs, totalRTT, totalJitter);

    cout << "\nTotal RTT (sum) calculated using PIM: " << totalRTT << " ms" << endl;
    cout << "Total Jitter (sum) calculated using PIM: " << totalJitter << " ms" << endl;
